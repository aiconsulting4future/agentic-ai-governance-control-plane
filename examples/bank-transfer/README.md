...
Only after all identified AI-originated consequence-bearing routes within the declared scenario boundary are acceptably governed may the system declare:

```text
RouteClosure(Q) = TRUE
```

This declaration is bounded to the route inventory examined for the scenario. It is not evidence that every possible banking, infrastructure, administrative, or malicious path has been globally discovered.
...
For the completed reference scenario, assume all identified AI-originated routes within the declared scenario boundary have been remediated or equivalently governed.

This is a reference-scenario closure assumption over the enumerated route inventory, not an empirical claim of globally exhaustive path discovery.
...
Here, `CLOSED` means that the identified in-scope route set satisfies C11/C12. If route discovery were incomplete or insufficiently evidenced, the scenario should not use `CLOSED` as proof that no undiscovered bypass exists.
